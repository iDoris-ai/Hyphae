...
- **`scripts/deploy-relay.sh check` 有两个非阻塞的边界 case**（PR #23 review 时，本仓库后台 review bot 自己的对抗性测试发现，均已复现确认，判定为 Low、不阻塞合并）：(a) `check_port` 的范围校验（`[ "$port" -lt 1 ] || [ "$port" -gt 65535 ]`）对超出 bash 64 位整数范围的纯数字字符串（比如 27 个 9）会失效——两个比较都会往 stderr 抛 `integer expression expected` 且各自判定为"假"，`||` 链条永远走不到"非法"分支，脚本会继续往下走到 `port_in_use`，最终误报"✅ free"，正好是本任务 round 1 修的那个 bug 类别（"非法端口被误报为空闲"）的另一种触发方式，只是这次是"数字但超范围"而不是"非数字/普通超范围"。修复思路：在数值比较前先限制字符串长度（比如超过 5 位直接判非法）。(b) `check_relay_repo` 的 `git ls-remote --exit-code "$RELAY_REPO"` 没有显式超时——针对一个真正 blackhole（丢包不拒绝）而非立即拒绝连接的地址，复现时跑了 8 秒以上都没有停，只受 OS 自己的 TCP connect 超时（常见 60-120 秒以上）约束，不是脚本自己控制的。对于一个标榜"快速 preflight 诊断"的工具，遇到这种网络场景可能会静默卡很久。建议后续给 `git ls-remote` 加显式超时（比如 `timeout 10s git ls-remote ...`，注意 macOS 默认没有 `timeout` 命令，需要用 GNU coreutils 的 `gtimeout` 或者其他等效机制）。
