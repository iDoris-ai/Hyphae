...
**创建 `AuraAIHQ/relay-khatru` 这个 GitHub 仓库本身不属于本任务**，那是一次性的人类基础设施操作（建仓库、配 CI、决定归属组织）。本任务只做 hyphae 这边、`scripts/deploy-relay.sh` 相关的准备工作，让"未来某天人类建好了 fork 仓库"这件事发生时，切换成本降到改一个环境变量的程度。如果 loop 执行到这个任务时发现需要真的去创建仓库才能验收，**说明任务范围理解错了，应该暂停并跳过這个验收点，只验收脚本本身的健壮性**。
